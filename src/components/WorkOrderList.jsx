...
const WorkOrderList = () => {
    ...
    return (
      ...
      <div>
        ...
        workOrders.filter(orders => orders.worker.name.includes(searchTerm)) 
        //Sorts the work orders based on the checkbox
        .sort((x, y) => { 
            return isSortByLatest ? (y.deadline - x.deadline) : (x.deadline - y.deadline);
        }) 
        .map((workOrder) => {
...
async function loadWorkOrders(setWorkOrders) {
  ...
  async function loadWorkerDetails(workOrder) {
    const url = 'https://api.hatchways.io/assessment/workers/' + workOrder.workerId;
    return fetch(url)
      .then(response => response.json())
      .then(data => {
